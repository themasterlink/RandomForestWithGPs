...
void cmaes_boundary_transformation_init(cmaes_boundary_transformation_t *t,
    ...
    t->lower_bounds = new_double(t->len_of_bounds);
    t->upper_bounds = new_double(t->len_of_bounds);
    ...
    if(upper_bounds != nullptr && lower_bounds != nullptr){
    	for(unsigned int i = 0; i < t->len_of_bounds; ++i){
    		t->lower_bounds[i] = lower_bounds[i];
    		t->upper_bounds[i] = upper_bounds[i];
    	}
    }
...
/*
void _manage_len(cmaes_boundary_transformation_t *t, unsigned int len)
{
...
*/
unsigned long _index(cmaes_boundary_transformation_t *t, unsigned long i)
{
