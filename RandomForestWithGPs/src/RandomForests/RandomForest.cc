...
RandomForest::RandomForest(const int maxDepth, const int amountOfTrees, const int amountOfClasses):
	m_maxDepth(maxDepth), m_amountOfTrees(amountOfTrees), m_amountOfClasses(amountOfClasses), m_counterIncreaseValue(2), m_trees(amountOfTrees, DecisionTree(maxDepth, amountOfClasses)){
}
...
void RandomForest::trainInParallel(const Data& data, const Labels& labels, const int amountOfUsedDims, RandomNumberGeneratorForDT& generator, const int start, const int end, TreeCounter* counter){
...
int RandomForest::predict(const DataElement& point) const{
	...
	if(!(m_amountOfTrees < nrOfParallel || true)){
		...
		boost::thread_group group;
		std::vector<std::vector<int> > vectors(nrOfParallel, std::vector<int>(m_amountOfClasses,0));
		for(int i = 0; i < nrOfParallel; ++i){
			const int start = (i/(double)nrOfParallel) * m_amountOfTrees;
			const int end =  ((i+1)/(double)nrOfParallel) * m_amountOfTrees;
		...
		return std::distance(vectors[0].cbegin(), std::max_element(vectors[0].cbegin(), vectors[0].cend()));
...
void RandomForest::predictInParallel(const DataElement& point, std::vector<int>& values, const int start, const int end) const{
	for(int i = start; i < end; ++i){
