...
OtherRandomForest::OtherRandomForest(const int maxDepth, const int amountOfTrees,
		const int amountOfClasses)
		:	m_maxDepth(maxDepth),
			m_amountOfTrees(amountOfTrees),
			m_amountOfClasses(amountOfClasses),
			m_counterIncreaseValue(2),
			m_trees(amountOfTrees, OtherDecisionTree(maxDepth, amountOfClasses)){
}
...
void OtherRandomForest::trainInParallel(const Data& data, const Labels& labels,
		const int amountOfUsedDims, RandomNumberGeneratorForDT& generator, const int start,
		const int end, OtherTreeCounter* counter){
...
int OtherRandomForest::predict(const DataElement& point) const{
	...
	for(std::vector<OtherDecisionTree>::const_iterator it = m_trees.cbegin(); it != m_trees.cend();
			++it){
...
void OtherRandomForest::predictDataInParallel(const Data& points, Labels* labels, const int start,
		const int end) const{
	for(int i = start; i < end; ++i){
